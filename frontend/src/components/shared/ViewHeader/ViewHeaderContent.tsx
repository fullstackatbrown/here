...
const ViewHeaderContent: FC<ViewHeaderContentProps> = ({ view, views }) => {
  ...
  return (
    <>
      ...
      <Button
        color="inherit"
        variant="text"
        sx={{ fontSize: 18, fontWeight: 600, ml: -1, display: { md: "none" } }}
        endIcon={<MenuIcon />}
        onClick={handleClick}
      >
        {capitalizeWords(view)}
      </Button>
      <StyledMenu anchorEl={anchorEl} open={open} onClose={handleClose} sx={{ display: { md: "none" } }}>
        {views.map((view) => {
          return (
            <MenuItem
              key={view}
              sx={{ fontSize: 18, py: 0, mr: 2 }}
              onClick={() => {
                handleClose();
                navigateTo(view);
              }}
