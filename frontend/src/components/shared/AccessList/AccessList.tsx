  ...
  const handleRevokeUserAccess = (user: User) => {
    return () => {
      const confirmed = confirm(
        `Are you sure you want to revoke ${user.displayName}'s ${access.toLowerCase()} access?`
      );
      if (confirmed) {
        toast
          .promise(CourseAPI.revokePermission(course.ID, user.ID), {
    ...
    <Stack direction="row" alignItems="start">
      <Box width={60} mt={0.5}>
        <Typography color="secondary" fontSize={14}>
          {capitalizeFirstLetter(access)}
        </Typography>
      </Box>
      {data && (
        <Box display="flex" flexWrap="wrap" flexDirection="row" alignItems="center">
          {data.length === 0 ? (
            <Typography mx={0.5} my={0.5} color="text.secondary" fontSize={14}>
              No {access.toLowerCase()} added yet
            </Typography>
          ) : (
            data.map((data, idx) => {
              if (data.user) {
                return (
                  <Tooltip key={idx} title={data.user.email} placement="right">
                    <Chip
                      variant="outlined"
                ...
                return (
                  <Tooltip key={idx} title={"pending"} placement="right">
                    <Chip sx={{ height: 26, marginRight: 0.5 }} label={data.email} size="small" />
                  </Tooltip>
                );
              }
            })
